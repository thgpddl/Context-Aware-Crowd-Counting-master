...
class CANNet(nn.Module):
    def __init__(self, load_weights=False):
        ...
        if not load_weights:
            ...
            # HYJ:将mod中的预训练参数加载到frontend中
            struce_dict=self.frontend.state_dict()
            pretrained_dict = {k[9:]: v for k, v in mod.state_dict().items() if k[9:] in struce_dict}
            struce_dict.update(pretrained_dict)
            self.frontend.load_state_dict(struce_dict)
